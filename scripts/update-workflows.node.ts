...
async function main(): Promise<void> {
  ...
  const { workflowUses, miseVersionUses } = await collectWorkflowInventory(workflowFiles);
  ...
  const updatesByKey = new Map(
    updates.map((update) => [groupKey(update.depName, update.currentVersion), update]),
  );
  ...
  for (const workflowUse of workflowUses) {
    const update = updatesByKey.get(groupKey(workflowUse.actionName, workflowUse.currentVersion));
  ...
  for (const miseVersionUse of miseVersionUses) {
    ...
    const fileLines =
      ...
      (await readFile(miseVersionUse.filePath, `utf8`)).split(/\r?\n/);
    fileLines[miseVersionUse.lineIndex] = `${miseVersionUse.prefix}${update.targetVersion}`;
...
async function listWorkflowFiles(workspaceRoot: string): Promise<string[]> {
  ...
  const filePaths = await listFiles(githubDir);
...
async function collectWorkflowInventory(filePaths: string[]): Promise<WorkflowInventory> {
...
function groupMiseVersionUses(miseVersionUses: MiseVersionUse[]): MiseVersionGroup[] {
...
async function resolveUpdates(groups: ActionGroup[]): Promise<ResolvedUpdate[]> {
  ...
  return Promise.all(
    groups.map(async (group) => {
      ...
      const hasUpdate = group.currentRef !== targetRef || group.currentVersion !== targetVersion;
...
async function resolveMiseUpdates(groups: MiseVersionGroup[]): Promise<ResolvedMiseUpdate[]> {
  if (groups.length === 0) {
    return [];
  }
...
function selectLatestTag(tags: string[]): string | null {
  const parsedTags = tags
    ...
    .filter((entry): entry is { tag: string; parsed: ParsedVersion } => entry.parsed !== null)
...
async function resolveTagCommit(repository: string, tag: string): Promise<string> {
  const remote = `https://github.com/${repository}.git`;
  const { stdout: peeledOutput } = await execFileAsync(`git`, [
    `ls-remote`,
    remote,
    `refs/tags/${tag}^{}`,
  ]);
  ...
  const { stdout: directOutput } = await execFileAsync(`git`, [
    `ls-remote`,
    remote,
    `refs/tags/${tag}`,
  ]);
...
function findMiseVersionUse(
  ...
  for (let lineIndex = actionLineIndex + 1; lineIndex < fileLines.length; lineIndex += 1) {
